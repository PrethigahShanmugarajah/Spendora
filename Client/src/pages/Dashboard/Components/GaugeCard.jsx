...
const GaugeCard = ({
  ...
  const gradientStart = isNegative
    ? "#F43F5E"
    : colorInfo.gradientStart || "#10B981";
  ...
  const percentColor = isNegative ? "text-rose-500" : "text-gray-500";
    ...
    <div className="bg-white rounded-xl p-5 -mx-3 lg:mx-0 md:-mx-5 shadow-sm flex flex-col items-center border border-gray-100">
      ...
      {loading ? (
        <div className="w-full h-28 flex items-center justify-center">
          <MoonLoader size={50} color={gradientStart} />
        </div>
      ) : (
      ...
      <div className="text-center mt-3">
        ...
        <p className="text-sm text-gray-500 flex items-center justify-center">
          {loading ? (
            <BeatLoader size={6} color={gradientStart} />
          ) : (
            `${timeFrameLabel} data`
          )}
        </p>
